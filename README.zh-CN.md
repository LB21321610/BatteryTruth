...
支持 macOS 14 到 macOS 26。仓库仍保留 [`macos14/`](macos14/) 兼容目录，方便单独验证旧系统构建。🍃
...
- 🪟 **原生 SwiftUI 应用**：Apple 官网式浅色产品页界面、全窗口布局、顺滑滚动、菜单栏显示
...
BatteryTruth **不会伪造充电控制**。在 macOS 14 到 macOS 26 上，本 App 没有可直接调用的公开接口来切断充电。

App 内的充电上限和热保护功能，是基于真实电池数据的监测和提示功能。越过阈值时会发送 macOS 本地通知；它们是有用的信号，不是隐藏的系统级断充开关。🛡️

## 界面感觉 🌌

BatteryTruth 的界面改为 Apple 官网式浅色产品页：大号实时数字、干净留白、浅色圆角模块、可视化电池图标、健康度环、原始容量面板、功率面板，以及直接放在主界面里的设置选项。
